<?php
class ci_cd_disposiciones extends docu_ci
{
    //-------------------------------------------------------------------------
    function relacion()
    {
    ...
    function conf__cuadro(docu_ei_cuadro $cuadro)
    {
        ...
        $datos = toba::consulta_php('co_consejodirectivo')->get_cd_disposiciones($where);
        foreach ($datos as $dat) {
            if ($dat['archivo_path'] != '') {
                ...
                $dat['disposicion'] = "<a href='{$temp_archivo['url']}'target='_blank'>".$dat['disposicion_numero']."</a>";    
            }
            $aux[] = $dat;
        }
    ...
    function conf__form(docu_ei_formulario $form)
    {
        $fecha = new DateTime();
        $fecha->getTimestamp();
        $datos = $this->tabla('cd_disposiciones')->get();
    ...
    function evt__form__modificacion($datos)
    {
        if (isset($datos['archivo']) or isset($datos['archivo_path'])) {
            ...
            $nombre_nuevo = 'disp_'.$datos['ciclo_lectivo'].'_'.$datos['disposicion_numero'].'.pdf';   
